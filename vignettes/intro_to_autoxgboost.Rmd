...
The goal of our analysis is to equally optimize the mean missclassification error as well as a fairness measure.
In this case, we defien a fairness measure as our model's absolute difference in F1-Scores between two groups.
...
```{r, eval = FALSE}
set.seed(20190606)
axgb$fit(iterations = 20L, plot = FALSE)
```
...
A big problem in human-in-the-loop approaches is often the reproduciblity of obtained results.
In our case, if we set a seed before starting our analysis, and a user provides the code used
in order to intervene in the process, the process is fully reproducible.
